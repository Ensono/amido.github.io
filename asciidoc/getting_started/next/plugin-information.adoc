...
An existing https://nextjs.org/[Next] application. Note the generator will fail if run in an empty workspace with no applications. To create a new Next application please run the NX Next generator with the following command including any relevant options. See https://nx.dev/packages/next[@nx/next plugin docs]
...
include::next-auth-redis.adoc[]
include::next-auth-redis-deployment.adoc[]

...
> **Note**
>
> If no template is specified for the `variable` then the static version built into the CLI will be used.
...
In this example all of the values for the parameters come from the `.Input` or `.Project` object (as detailed in the [Structure](#structure) section).

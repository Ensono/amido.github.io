...
The default ingest workload generated by Datastacks is based upon ingesting from an Azure SQL data source. For the purposes of the _getting started_ example to you can leave the resources generated as they are. See [ingest data source types](../etl_pipelines/ingest_data_azure.md#data-source-types) for further information on adapting the workload for other data source types.

:::

:::tip

The default ingest workload contains an example [tumbling window trigger](../etl_pipelines/ingest_data_azure.md#data-factory-triggers), which defaults to a 'Stopped' state. This is defined in the Terraform resource in `data_factory/adf_triggers/tf`, and can be modified based on your requirements.
